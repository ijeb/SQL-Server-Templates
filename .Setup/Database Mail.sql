...
-- Account and SQL Operator information. Replace with the information for your account.
	...
	SET @SMTP_servername = N'<Mailserver, sysname, mail.ram.nl>';
	SET @email_address = N'<E-mail address, sysname, dbateam@ram.nl>';
    SET @display_name = N'<Display name, sysname, @@servername>';
	SET @port_number = 25;
	SET @desc_p = 'Mail account used by DBA staff';
	SET @desc_a = 'Mail account used by DBA staff';
	SET @customoper = N'<Operator, sysname, DBA>'

...
SELECT @@servername servername
	...
	,c.session_id
	,case when p.dbid=0 then N'' else ISNULL(db_name(p.dbid),N'') end [database_context_current]
	,w.name [group]
	,s.[program_name]
...
SELECT servername, database_name, [db_containment], username, [user_containment], login, login_type, authentication_type_desc, [database_context_current], [group], net_transport, auth_scheme, [program_name], client_interface_name, client_net_address, [host_name],
local_net_address, local_tcp_port, listener_dns_name, COUNT(session_id) as #_connections,
...
GROUP BY servername, database_name, [db_containment], username, [user_containment], login, login_type, authentication_type_desc, [database_context_current], [group], net_transport, auth_scheme, [program_name], client_interface_name, client_net_address, [host_name],
local_net_address, local_tcp_port, listener_dns_name, server_roles, [server_permissions], database_roles, [database_permissions]
